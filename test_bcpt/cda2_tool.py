...
def get_driveid(drivename, report_fn=None, create=True):
    ...
    if create:
        c.execute(
            "INSERT INTO drives (driveid,drivename,report_fn) VALUES (NULL,?,?)",
            (drivename, report_fn),
        )
...
def get_featureid(feature):
    c = conn.cursor()
    c.execute(
        "INSERT OR IGNORE INTO features (featureid,feature) VALUES (NULL,?)", (feature,)
    )
...
def list_drives():
    ...
    for rowid, drivename in c.execute("SELECT rowid,drivename FROM drives"):
        print(f"{rowid:3} {drivename}")


def feature_drive_count(featureid):
    assert isinstance(featureid, int)
    c = conn.cursor()
    c.execute(
        "SELECT count(*) from feature_drive_counts where featureid=? ", (featureid,)
    )
    return c.fetchone()[0]


def ingest(report_fn, args):

    c = conn.cursor()
    c.execute("select count(*) from drives where report_fn=?", (report_fn,))
    if c.fetchone()[0] > 0 and args.reimport is False:
    ...
    for search, count in br.read_histogram_entries("url_searches.txt"):
        if search.startswith(b"cache:"):
            continue
        featureid = get_featureid(search)
        c.execute(
            "INSERT INTO feature_drive_counts (driveid,feature_type,featureid,count) values (?,?,?,?);",
...
def correlate_for_type(driveid, feature_type, args, verbose=True, larger=False):
    ...
    def feature_fmt(feature):
        ...
        if (feature_type == SEARCH_TYPE) and ("%" in feature):
            try:
                feature = (
                    f"{feature} ({urllib.parse.unquote(feature, errors='strict')})"
                )
    ...
    res = []
    c.execute(
        "SELECT R.drivecount,C.featureid,feature FROM feature_drive_counts as C JOIN features as F ON C.featureid = F.rowid "
    ...
    for line in res:
        ...
        for (driveid_,) in c.execute(cmd, arg):
            if args.debug:
                print(f"  also on drive {driveid_}")
            if driveid_ not in coefs:
                coefs[driveid_] = 0
                contribs[driveid_] = []
    ...
    if verbose:
        for driveid_, coef in sorted(coefs.items(), key=lambda a: a[1], reverse=True):
            print(f"Drive {driveid_} {get_drivename(driveid_)}")
            print(f"Correlation: {coef:.6}")
            for weight, featureid, feature in sorted(contribs[driveid_], reverse=True):
                print(f"   {weight:.2}   {feature_fmt(feature)}")
            print("")
    return coefs


def make_report(driveid, args, verbose=True):
    c = conn.cursor()
    ...
    if args.email:
        ...
        correlate_for_type(driveid, EMAIL_TYPE, args, verbose=verbose)
    if args.search:
        print("Search correlation report:")
        correlate_for_type(driveid, SEARCH_TYPE, args, verbose=verbose)
    if args.winpe:
        print("WINPE correlation report:")
        correlate_for_type(driveid, WINPE_TYPE, args, verbose=verbose)
...
def build_feature_frequencies():
    ...
    c.execute(
        "insert into feature_frequencies (featureid,feature_type,drivecount,featurecount) select featureid,feature_type,count(*),sum(count) from feature_drive_counts group by featureid,feature_type"
    )
    conn.commit()
...
def main():
    ...
    parser.add_argument(
        "--list", help="List the reports in the database", action="store_true"
    )
    parser.add_argument(
        "--recalc",
        help="Recalculate all of the feature counts in database",
        action="store_true",
    )
    parser.add_argument("--test", help="Test the script", action="store_true")
    parser.add_argument(
        "--report", help="Generate a report for a specific driveid", type=int
    )
    parser.add_argument("--build", help="build feature_frequences", action="store_true")
    parser.add_argument(
        "--drive_threshold",
        type=int,
        help="don't show features on more than this number of drives",
        default=10,
    )
    parser.add_argument(
        "--correlation_cutoff",
        type=float,
        help="don't show correlation drives for coefficient less than this",
        default=0.5,
    )
    parser.add_argument(
        "--reimport",
        help="reimport drives, rather than ignoring drives already specified",
        action="store_true",
    )
    parser.add_argument(
        "--all",
        help="calculate the correlation coefficient for all",
        action="store_true",
    )
    ...
    parser.add_argument(
        "--search", help="Correlate on search terms", action="store_true"
    )
    parser.add_argument(
        "--winpe", help="Correlate on windows executables", action="store_true"
    )
    ...
    if args.all:
        ...
        for drive1 in range(start, 2000):
            corr = correlate_for_type(
                drive1, SEARCH_TYPE, args, verbose=False, larger=True
            )
            print(corr)
            c.execute("delete from drive_correlations where (driveid1=?)", (drive1,))
            for drive2, corr in corr.items():
                c.execute(
                    "insert into drive_correlations (driveid1,driveid2,corr) values (?,?,?)",
                    (drive1, drive2, corr),
                )
                print(drive2, corr)
            conn.commit()
