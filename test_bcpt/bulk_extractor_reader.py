...
def parse_feature_line(line):
    ...
    if len(line) < 2:
        return None
    if line[0] == b"#":
        return None  # can't parse a comment
    if line[-1:] == b"\r":
        line = line[:-1]  # remove \r if it is present (running on Windows?)
    ary = line.split(b"\t")
    ...
    if (
        len(ary) < MIN_FIELDS_PER_FEATURE_FILE_LINE
        or len(ary) > MAX_FIELDS_PER_FEATURE_FILE_LINE
    ):
        # Don't know
        return None
    if b"\xf4\x80\x80\x9c" in ary[0]:
        return ary  # contains files
    if len(ary[0]) < 1:
        return None
    if ary[0][0] < ord("0") or ary[0][0] > ord("9"):
        return None
    return ary
...
def is_histogram_filename(fname):
    ...
    if "_histogram" in fname:
        return True
    if "url_" in fname:
        return True  # I know this is a histogram file
    if fname == "ccn_track2.txt":
        return False  # I know this is not
...
def is_feature_filename(fname):
    ...
    if not fname.endswith(".txt"):
        return False
    if "/" in fname:
        return False  # must be in root directory
    if "_histogram" in fname:
        return False
    if "_stopped" in fname:
        return False
    if "_tags" in fname:
        return False
    if "wordlist" in fname:
        return False
...
class BulkReport:
    ...
    def image_filename(self):
        ...
        return self.xmldoc.getElementsByTagName("image_filename")[
            0
    ...
    def image_size(self):
        ...
        return int(
            self.xmldoc.getElementsByTagName("image_size")[0].firstChild.wholeText
        )
    ...
    def threads(self):
        ...
        return int(
            (
                self.xmldoc.getElementsByTagName("configuration")[0]
                .getElementsByTagName("threads")[0]
                .firstChild.wholeText
            )
        )
    ...
    def page_size(self):
        ...
        return int(
            (
                self.xmldoc.getElementsByTagName("configuration")[0]
                .getElementsByTagName("pagesize")[0]
                .firstChild.wholeText
            )
        )
    ...
    def margin_size(self):
        ...
        return int(
            (
                self.xmldoc.getElementsByTagName("configuration")[0]
                .getElementsByTagName("marginsize")[0]
                .firstChild.wholeText
            )
        )
    ...
    def clocktime(self):
        ...
        return float(
            (
                self.xmldoc.getElementsByTagName("rusage")[0]
                .getElementsByTagName("clocktime")[0]
                .firstChild.wholeText
            )
        )
    ...
    def cpu_track(self):
        ...
        return sorted(
            [
                ...
                for e in self.xmldoc.getElementsByTagName("runtime")[
                    0
                ].getElementsByTagName("debug:cpu_benchmark")
            ]
        )
    ...
    def is_histogram_file(self, fn):
        if is_histogram_filename(fn) == True:
            return True
